...
function createDeserialiser (factory, spec, opts) {
  validateSpec(spec)
  const keys = Object.keys(spec)

  const _opts = {
    treatNullAsUndefined: opts && opts.treatNullAsUndefined === true
  }

  const deserialiser = function (serialisedData, initialisationData) {
    const model = factory(initialisationData)

    function _doTheSoulfulChicken (key) {
      const val = getValueForKeyFromDataBasedOnSpec(key, spec[key], serialisedData)

      if (val !== undefined && (_opts.treatNullAsUndefined === false || val !== null )) {
        setValueForKeyBasedOnSpec(key, spec[key], val, model)
      }
    }
